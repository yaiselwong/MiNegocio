...
namespace MiNegocio.Server.Services
{
    public class WarehouseManagementService : IWarehouseManagementService
    {
        ...
        public WarehouseManagementService(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<WarehouseDto> CreateWarehouseAsync(CreateWarehouseRequest request, int companyId)
        {
            ...
            return await GetWarehouseByIdAsync(warehouse.Id) ?? throw new Exception("Error creating warehouse");
        }

        public async Task<WarehouseDto?> UpdateWarehouseAsync(int id, CreateWarehouseRequest request)
        {
            var warehouse = await _unitOfWork.WarehouseRepository.FindOneAsync(w => w.Id == id);
